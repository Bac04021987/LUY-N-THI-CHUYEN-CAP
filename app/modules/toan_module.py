...
class ToanModule:
    ...
    xử lý bài tập chi tiết, và có cơ chế cache hiệu quả.
    """

    def __init__(self, language: str = "vi"):
        """
        :param language: mã ngôn ngữ (mặc định 'vi' - tiếng Việt)
        """
    ...
    def get_tests_by_level(self, level: int) -> List[Dict]:
        ...
        # Phân biệt thư mục data theo ngôn ngữ (dự phòng mở rộng)
    ...
    def get_test_description(self, test: Dict) -> str:
        ...
        try:
            ...
            # Có thể bổ sung bước kiểm tra đơn giản mô tả đầu ra (ví dụ độ dài, từ khóa)
            if not response or len(response) < 10:
                logger.warning(f"Mô tả đề thi quá ngắn hoặc không hợp lệ: {response}")
                return "Mô tả đề thi không đủ chi tiết."
